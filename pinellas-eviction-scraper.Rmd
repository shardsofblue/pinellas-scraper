...
check_for_element <- function(xpath) {
  if(exists("remote_driver")){
    # Get page HTML source code (stores as list)
    pagehtml <- remote_driver$getPageSource()
    # Read in html as nodes
...
view_case <- function(x){
  if(check_for_element(xpath = "/html/body/table[4]/tbody/tr[3]/td[1]/a") == TRUE) {
    view_case_action(x)
    } else {
      # Go to the home page and log back in
      login_to_pinellas_courts()
...
return_to_results <- function(){
  if(check_for_element(xpath = "/html/body/table[2]/tbody/tr/td/table/tbody/tr/td[1]/font/a[7]")){
...
for(i in 1:nrow(cases_df_clean)) {
  print(i)
  # This is where I put select_case(i)
  # I must also change items that are FLAGed at this step
}
